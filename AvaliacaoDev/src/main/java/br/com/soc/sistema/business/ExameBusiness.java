...
public class ExameBusiness {
	...
	public void salvarExame(ExameVo exameVo) {
		try {
			...
			throw new BusinessException("Não foi possivel realizar a inclusao do registro");
		}
		
	}	
	
	public void deletarExame(String codigo) throws BusinessException{
		try {
			boolean exist = dao.findExistInExameFucionario(codigo);
			
			if(!exist) {
				try {
					dao.deleteExame(codigo);
				}catch (Exception e) {
					throw new BusinessException("Nao foi possivel excluir o exame");
				}
			}
		} catch (Exception e) {
			throw new BusinessException("Nao foi possivel excluir o exame");
	...
	public void editarExame(ExameVo exameVo) throws BusinessException{
		try {
			if(exameVo.getNome().isEmpty())
				throw new IllegalArgumentException("Nome não pode ser em branco");
